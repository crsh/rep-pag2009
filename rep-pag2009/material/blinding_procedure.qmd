...
To accomodate the hierarchical nature of the data, I fit the lognormal model developed above to each participant individually to equalize the average decision times $\mu$. 

First, I simulated data for 50 from a three-parameter log-normal model to demonstrate the general approach.
...
filter(synth_dat_peek, id %in% c(0, 1)) |>
  ggplot() +
...
```{r}
#| label: fit-hierarchical-log-normal-model
...
ndt_inits <- list(Intercept_ndt = -5)
...
mod_synth <- brm(
  ...
  , iter = 1500
  , warmup = 1000
  , init = replicate(3, ndt_inits, simplify = FALSE)
  , thin = 1
...
synth_dat |>
  ...
  summarize(
    sln = fitdist(
      ...
      , "shifted_lnorm"
      , method = "mle"
      , start = list(meanlog = -0.5, sdlog = 0.5, shift = 0.05)
    ) |> coef() |> t() |> as.data.frame()
    , .by = c(id, correct, condition)
  ) |>
  tidyr::unnest(cols = sln) |>View()
